...
Three hand-authored HTML reference docs live at the repo root. They are the curated source of truth for *what the game currently is* — consult them before working on UI/systems, and **update them in the same change whenever the thing they describe changes**.
...
- **`sistemas-del-juego.html`** — tabbed reference for the systems actually live in the main loop (Forum hub + Iter Belli). Reflects ground truth: faith/influence/momentum are deprecated (only Aurum/Iuniores/Season are live); Iter Belli is the card-driven Saguntum campaign (not the legacy hex Bellum). Keep it accurate if systems change.
...
## Project Structure
- `src/game/core/` — commander, game-state, meta-save, resources (only gold/iuniores/season live; faith/influence/momentum are `@deprecated`, kept for save compat)
