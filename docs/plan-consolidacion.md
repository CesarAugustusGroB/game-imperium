...
- **Nota:** traits de legado NO usan una unión de efecto-tipo tabulada (son mods bespoke en el adapter); su contrato no aplica al patrón de este verificador.
- **Ampliación (it.29–30):** `verify-effects.ts` ahora cubre 5 uniones: `DoctrineEffect`, `DecretumEffect`, `TradeGoodSpecial`, `FeatureSpecial` y `GovernorTrait`. Cada tipo debe estar registrado como **aplicado** (`files`) o **latente** (`{ latent: true }`, dato inerte documentado). Esto cazó la clase del bug `enables-building` (it.28): un tipo descrito pero no aplicado ahora falla el guard salvo que se marque latente con motivo. Los 6 traits de gobernador verificados todos vivos.
