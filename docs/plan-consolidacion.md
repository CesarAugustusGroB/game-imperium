...
- **Nota:** traits de legado / governors NO usan una unión de efecto-tipo tabulada (son mods bespoke en el adapter / province); su contrato no aplica al patrón de este verificador.
- **Ampliación (it.29):** `verify-effects.ts` ahora cubre también las uniones `TradeGoodSpecial` y `FeatureSpecial` de provincia. Cada tipo debe estar registrado como **aplicado** (`files`) o **latente** (`{ latent: true }`, dato inerte documentado). Esto cazó la clase del bug `enables-building` (it.28): un tipo descrito pero no aplicado ahora falla el guard salvo que se marque latente con motivo.
