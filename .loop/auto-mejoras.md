...
- [BLOCKED: el MECANISMO está resuelto (añadir `aiSmart?: boolean` a BattleState, fijado por el controller en Dura; `enemyChoose(S)` lee `S.aiSmart` → ruta determinista actual intacta cuando es false, así que Normal + los tests `enemyChoose(s)` no cambian). Lo que falta es DECISIÓN DE BALANCE: qué contra-juego/aleatoriedad añadir y cuánto más dura debe quedar la IA en Dura. El sim (sim-battle-balance.ts) NO ejercita la IA smart, así que no puedo validar el win-rate sin tunearlo/decidirlo. ¿Cómo de lista quieres la IA en Dura, y conservadora o agresiva?] DF2 · IA enemiga menos explotable. (Topic 10)
...
- DF2 → BLOCKED (decisión de balance: comportamiento de la IA en Dura; mecanismo de integración ya resuelto en la nota). IDLE: 0 pasos READY. Heartbeat verde (tsc+verify 18/18+build). 22 pasos completados; todo lo restante espera decisión/spec del usuario (QW2b, BG3b, DF1c, DF2, BG1, BG2) o es opcional (BG3c).
