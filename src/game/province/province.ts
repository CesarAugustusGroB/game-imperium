...
import {
  FOOD, GROWTH, TAX, WEALTH, SETTLEMENT, PROVINCE_DEFAULTS, IUNIORES,
...
/**
 * Gross income a province generates each season — the single source of truth
 * shared by the season tick (`collectProvinceIncome`) and every income display,
 * so the ledger never disagrees with what the tick actually pays out.
 *
 * = tax revenue (wealth × tax rate) + building gold + 1 subsistence + synergy
 *   gold + flat non-gold building output + trade-good and unique-feature flats
 *   + the population iuniores yield, with governor income-bonus traits applied
 *   per resource. Empire-wide modifiers (e.g. the Aqueduct +10%) are layered on
 *   by the caller, not here. Note: `province.baseIncome` is intentionally not
 *   used — the live economy derives gold from tax + buildings, not that field.
 */
export function getProvinceIncome(
  province: Province,
  governorTraits: GovernorTrait[] = [],
): Partial<Record<ResourceType, number>> {
  const taxRevenue = Math.floor(province.wealth * getTaxRate(province.lowerTax, province.upperTax));

  // Building income, split into the tax-bucket gold vs flat non-gold output.
  let buildingGold = 0;
  ...
  const income: Partial<Record<ResourceType, number>> = {
    gold: taxRevenue + buildingGold + 1, // +1 subsistence
  };
  for (const [res, amt] of Object.entries(nonGold) as [ResourceType, number][]) {
    income[res] = amt;
  }

  // Trade-good flat output (no tax scaling).
  if (province.tradeGood) {
    const good = TRADE_GOOD_DATA[province.tradeGood];
    if (good.flatGold > 0)     income.gold     = (income.gold     ?? 0) + good.flatGold;
    if (good.flatIuniores > 0) income.iuniores = (income.iuniores ?? 0) + good.flatIuniores;
  }

  // Unique feature flat output.
  const feat = province.uniqueFeature;
  if (feat) {
    if (feat.goldPerSeason)     income.gold     = (income.gold     ?? 0) + feat.goldPerSeason;
    if (feat.iunioresPerSeason) income.iuniores = (income.iuniores ?? 0) + feat.iunioresPerSeason;
  }

  // Iuniores yield from population.
  const iunioresYield = Math.floor(province.population * IUNIORES.perPop);
  if (iunioresYield > 0) {
    income.iuniores = (income.iuniores ?? 0) + iunioresYield;
  }

  // Governor income-bonus traits (percentage per resource).
  for (const trait of governorTraits) {
    if (trait.type === 'income-bonus' && income[trait.resource] != null) {
      income[trait.resource] = Math.floor(income[trait.resource]! * (1 + trait.percent / 100));
    }
  }
