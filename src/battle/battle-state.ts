...
export class BattleState {
   ...
   * BattleUnits via the cohort→BattleUnit mapper.
   *
   * S15-04: blue with no cohorts spawns nothing (embark gate prevents this).
   * Red always receives a generated army from `generateEnemyArmy()`.
   * The canonical formation is kept as a private fallback but is no longer
   * reached through normal gameplay.
   *
   ...
   * trait pass (if any).
   *
   * S15-04: cohort-driven spawning only — no canonical fallback. If army
   * has no cohorts (or is missing), that faction starts with zero units.
   * The embark gate (S15-01) prevents blue from entering battle without
   * cohorts; red always receives a generated army from the caller.
   */
  private placeFactionUnits(
    ...
    // No fallback — zero units if army is missing or empty
    if (legate) {
  ...
  // placeCanonicalFormation removed in S15-04 — enemy armies are now
  // generated by generateEnemyArmy(), blue requires cohorts via embark gate.
}
